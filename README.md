...
| **Brush Engine** | Round, Soft, Flat, Scatter, Pixel presets with adjustable size, opacity, and flow; bounded `.abr` brush-set import adds persistent raster-stamp presets with spacing, opacity, scatter, and pen-pressure sizing |
...
`npm run test:mobile` runs the compact-workspace capability matrix in Chromium, Firefox, and WebKit using Pixel 5 and iPhone 13 emulation. It checks touch/pinch behavior, pressure variance handling, safe-area capability reporting, visual-viewport resize signals, orientation signals, and narrow-layout overflow. `npm run test:release` adds a high/critical advisory gate before running the performance, unit, desktop, cross-browser, and mobile suites.
